...
4. **`boot-restore.sh` (on_boot.d hook)** — firmware updates reset `/etc` and root's crontab, which removes the service link, the enable link, and the cron jobs. Mechanisms 1 and 2 can't recover from that on their own: `setup.sh` only runs when something starts the service, and `ensure-rules.sh` only acts while the bouncer is running. `boot-restore.sh` re-links and enables the service, restores the cron jobs, and starts the bouncer. It runs on every boot through [on-boot-script-2.x](https://github.com/unifi-utilities/unifi-common) (`/data/on_boot.d/99-crowdsec-bouncer.sh`, added by `install.sh`). `install.sh` leaves an existing on-boot-script alone; if it's missing, it installs `udm-boot.service` from a pinned unifi-common commit after a SHA-256 check (mismatch aborts before any change). With `ONBOOT_AUTO_INSTALL=0` it skips that, and you run `boot-restore.sh --boot` by hand after an update.
